...
lemma forces_inl {x : M.World} : Forces (M := (M.toPseudoTail r o).toModel) (.inl x) A ↔ x ⊩ A := by
  induction A generalizing x with
  | box A ihA =>
    constructor;
    . intro h y Rxy;
      exact ihA.mp $ h (.inl y) (rel_inl_inl.mpr Rxy);
    . rintro h (y | i) Rxy;
      . exact ihA.mpr $ h y (rel_inl_inl.mp Rxy);
      . exact absurd Rxy not_rel_inl_inr;
  | _ => grind [Model.World.Forces];
